...
export interface LatestInstrument {
  slug: string
  /** The featured instrument's meta, with `date` narrowed: latestInstrument() has already
   * thrown on a missing committed date, so consumers (the /field page builds marks, rows and
   * the day range from it) get a plain `string` instead of re-checking `string | undefined`. */
