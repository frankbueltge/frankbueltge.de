...
export function buildGlobeFloorSvg({ land, arcs, satellites, points, labels }: FloorInput): string {
  ...
  const landPath = path(feature(land, landObject) as never) ?? ''
  const graticule = path(geoGraticule10()) ?? ''

  const s: string[] = []
  s.push(
    `<svg class="sky-floor-svg" viewBox="0 0 ${FLOOR_W} ${FLOOR_H}" role="img" aria-labelledby="sky-floor-title sky-floor-desc" xmlns="http://www.w3.org/2000/svg">`,
  )
  s.push(`<title id="sky-floor-title">${escapeXml(labels.title)}</title>`)
  s.push(`<desc id="sky-floor-desc">${escapeXml(labels.desc)}</desc>`)
  s.push(`<rect class="sky-sea" x="0" y="0" width="${FLOOR_W}" height="${FLOOR_H}"/>`)
  s.push(`<path class="sky-grat" d="${graticule}"/>`)
  s.push(`<path class="sky-land" d="${landPath}"/>`)
