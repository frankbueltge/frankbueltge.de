...
export const NOTATION_REGISTER: readonly NotationEntry[] = [
  ...
  },
  {
    ...
    openQuestions: [
      {
        date: '2026-08-03',
        note: 'The atelier has no named cast while the other two do, and the figure shows the absence rather than filling it. Is that the practice’s form, or a gap in its protocol that its own next revision will close? — Update 2026-08-08: not closed, but the answer moved. The architect culled the other two rosters toward this shape rather than filling this one, and cited the atelier’s absence as the house’s own evidence: it convenes nothing and holds the largest works/ of the three. The question that remains is narrower — whether a practice with no role machinery is a form worth the other two converging on, or a practice that has simply never needed one yet. **And a correction to this entry’s own first sentence, found while updating it:** the figure does not in fact show the cast. `cast` and `castNote` are modelled in anatomy.ts and checked by its test, but no component renders them — the drawn figure is the stage chains only. So the absence is recorded and verified, not displayed, and this question has been describing something no visitor could see since 2026-08-03. Whether to draw the rosters is now its own open question, deliberately left to Frank rather than answered inside a protocol amendment.',
