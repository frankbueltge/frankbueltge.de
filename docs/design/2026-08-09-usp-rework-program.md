...
| **The ecology lane of the graph** (Frank, 2026-08-09 afternoon, all four options): the practices' own 59 works + the encounters into `graph.json` · nightly `graph:build` workflow · the USP obligation extended to the ecology (first batch + dated remainder) · the attention export **contract**, consumer side only | this session (afternoon 2026-08-09), branch `feat/ecology-graph` | 2026-08-09 | building — touches `src/lib/graph/**`, `docs/audits/**`, `.github/workflows/graph.yml`. **Does NOT touch the `machine-attention` repo:** that lane is claimed above, so the export is specified here and consumed when its owner ships it |
