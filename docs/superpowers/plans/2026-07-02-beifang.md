...
**Interfaces:**
- Consumes: —
- Produces: `registrable_domain(host: str) -> str`; `parse_easyprivacy(text: str) -> frozenset[str]`; `TdsData(tracker_domains: frozenset[str], entity_map: dict[str, str])`; `parse_tds(tds: dict) -> TdsData`; `Classification(third_party_hosts: frozenset[str], tracker_hosts: frozenset[str], entities: frozenset[str])`; `classify(first_party_domain: str, hosts, easyprivacy, tds: TdsData) -> Classification`.
- **Amendment (2026-07-02, nach Task 3):** Das reale TDS führt Firmen wie LiveRamp nur in den breiten `domains`/`entities`-Sektionen, nicht in der engen `trackers`-Sektion. Methodische Trennung: **Tracker-Erkennung** = EasyPrivacy ∪ Keys der TDS-`trackers`-Sektion; **Firmen-Zuordnung** = breite Domain→Firma-Map aus `trackers`-Ownern ∪ `domains`-Sektion (displayName via `entities`-Sektion). Spec-konform (§4: „Tracker Radar … Domain → Firma"); im Methodenblatt (Task 12 §4) so ausweisen.
