...
**Status (updated 2026-08-15).** Index document for ten proposals, of which **two are
built**: The Balance is live at `/balance`, and Editorial Deadline's world chamber runs as
chamber 2 of `/redaction` — both shipped 2026-08-14, the day after this index was written.
The other eight are open. Each has its own spec; every spec ends in a cheap spike whose
result — not this document — decides whether the instrument is commissioned. Frank's
request (2026-08-13, late, wording private): work through all the specs, thoroughly and
step by step.
...
| `2026-08-14-the-invoked-past-design.md` | Which past the press summons today; memory decay under proof | Lab | **Widest verified daylight** (GDELT built the field, never the tool) | Open — **recommended next** |
...
## Already built (2026-08-14)

Two of the ten shipped the day after this index was written, both from the recommended
order below as it originally stood:
...
- **The Balance** (was #3) — live at `/balance` with its method sheet, and leading
  `/experiments` by recency.

## Recommended order for the remaining eight (a recommendation, not a decision)
...
2. **The Unwitnessed** — strongest moral fit with the counter-measurement line; matcher
   precision is the gate.
3. **Tomorrow's Names** — hand to Machine Attention through its own channel; the lab
   does not build the strong version.
4. **The Front** — the half-life of editorial attention, on the same sealing condition as
...
5. The remaining four hold as documented candidates: The Quieting and The Seismic Quiet
