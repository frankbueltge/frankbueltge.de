...
| | prediction | verdict |
|---|---|---|
| **P1** | the dial is a line, slope 0.045–0.055, R² ≥ 0.99 | **held on arXiv, half-failed on Crossref** |
...
**P1.** Through the origin: arXiv slope **0.04691** (R² 0.99978), Crossref **0.04264**
(R² 0.99298). Both are lines across a sixteen-fold range of k in two literatures with nothing in
common. Crossref's slope falls outside the registered band.
...
One positive result falls out of P4's failure, and it is not obvious: **Benjamini-Hochberg is
self-correcting for exact duplicates.** A duplicated question adds one test to the denominator and
one small p-value to the numerator, and the two cancel.
...
empty world or a full one.

**Post-hoc, and marked so — this restriction was not pre-registered.** Over only the questions
that survive the loop's own review pre-conditions, the two rates are **4.87 %** (arXiv, 51
questions) and **4.94 %** (Crossref, 41 questions). The difference between the worlds vanishes and
both land on α. *The slope does transfer — between the questions that are awake.*
...
there. Both corpora are one day's fetch. The §6 restriction is post-hoc.
