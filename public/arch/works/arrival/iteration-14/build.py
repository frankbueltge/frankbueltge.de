#!/usr/bin/env python3
"""Build one instance of the work "Arrival", iteration 14, from public USGS data.
...
**What iteration 14 changes here: the block's name is checked, the way the
...
def build(eventid=None):
    ...
    bucket = {}
    for k in rings:
        bucket.setdefault((int(k[0] * 50), int(k[1] * 50)), []).append(k)
    for (bi, bj), ks in bucket.items():
        cand = []
        for di in (-1, 0, 1):
            for dj in (-1, 0, 1):
                cand.extend(bucket.get((bi + di, bj + dj), ()))
        for a in ks:
            for b in cand:
                if a >= b:
                    continue
                d = gc_km(a[0], a[1], b[0], b[1])
    ...
    # Over the catalogue this happens 2 680 times on 11 events; on the three
    # events this work was built on it has never happened once, which is why no
    # earlier iteration had to have a word for it.
    ever, returned = set(), 0
    for v in raw:
        add, chg, gone = [], [], []
        for key, b in v["blocks"].items():
        ...
        changed += len(chg)
        removed += len(gone)
        ever |= {order[k] for k in v["blocks"]}
