...
(async () => {
  ...
  for (const f of FILES) {
    for (const w of WIDTHS) {
      ...
      let maxGone = 0, sawGone = 0, maxNote = '';
      for (const [wi_, ri_, gi_, n_] of states) {
        const r = await page.evaluate(([a, b, c, d]) => {
          ...
          let wantGone = 0, wantHere = 0;
          for (const [i, s] of st) {
            if (s.n < minN) continue;
            if (s.gone) wantGone++; else wantHere++;
          }
          const felt = document.getElementById('felt');
          const gotGone = felt.querySelectorAll('rect.blkgone').length;
          const gotHere = felt.querySelectorAll('rect.blk').length;
          ...
          return {
            wantGone, wantHere, gotGone, gotHere, bad,
      ...
      console.log(`checked ${path.basename(f)} @${w}: ${states.length} states; withdrawn marks drawn in ${sawGone} of them, at most ${maxGone} — "${maxNote}"`);
      await ctx.close();
    }
  }
  await browser.close();
