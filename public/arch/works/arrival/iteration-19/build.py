#!/usr/bin/env python3
"""Build one instance of the work "Arrival", iteration 19, from public USGS data.
...
def build(eventid=None):
    ...
    # **What iteration 19 adds, and it is the only thing it adds.**
    #
    ...
    if len(epis) > 1 and ord_rows:
        ...
        for i in ord_st:
            ...
            if bk not in rings:
                continue
            lo0, hi0 = outline_edges(rings[bk], epis[0]["lat"], epis[0]["lon"])
            first.append(((lo0 + hi0) / 2.0, ord_c[i]))
        if len(first) == len(ord_rows):
            now_d = [d for d, _c in ord_rows]
            was_d = [d for d, _c in first]
            changed = pairs = 0
