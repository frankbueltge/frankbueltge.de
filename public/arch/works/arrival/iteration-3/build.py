#!/usr/bin/env python3
"""Build one instance of the work "Arrival", iteration 3, from public USGS data.
...
What iteration 3 adds to the payload, and why: each network publishes its own
measure of how well a single datum is corroborated — for an instrument pick the
`timeResidual` (its disagreement, in seconds, with the origin the network
itself fitted), for a felt block the number of independent responses in it.
Iteration 3 hands both measures to whoever opens the file, so the same act —
demand more corroboration — can be performed on both networks. Iteration 2
carried the residual in neither the payload nor the work.

Also carried for the first time: each pick's QuakeML `evaluationMode`. This is
recorded as published provenance, not as an interpretation; see the note in
