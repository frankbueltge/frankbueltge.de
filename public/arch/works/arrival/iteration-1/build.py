#!/usr/bin/env python3
"""Build one instance of the work "Arrival" from public USGS data.
...
def read_phases(detail):
    ...
    url = detail["properties"]["products"]["phase-data"][0]["contents"][
        "quakeml.xml"
    ]["url"]
...
def read_felt(detail):
    ...
    contents = detail["properties"]["products"]["dyfi"][0]["contents"]
    name = next(
...
def build(eventid=None):
    ...
    t0, phase_url, stations = read_phases(detail)
    felt_url, felt = read_felt(detail)
