#!/usr/bin/env python3
"""Build one instance of the work "Arrival", iteration 8, from public USGS data.
...
**What iteration 8 changes here: nothing.**

That is worth saying rather than passing over. Iteration 8 draws a quantity this
pipeline has been fetching, parsing and writing into every instance since the
first iteration — a block's published intensity — and which no iteration has
ever drawn. It travelled into the file, sat under the pointer as a line of
tooltip text, and was never given a position. So the correction iteration 8
makes is entirely in `template.html`, and this file is unchanged from
iteration 7 apart from this note. The change is not that the work now reads
something new; it is that the work now shows what it was already holding.

What forced it is measured in
...
never a point interpolated between two); and the degree/kilometre conversion
used for display. A block's published distance travels into the file beside its
computed ones, as in iterations 5 and 6, so the two coordinates can be compared
