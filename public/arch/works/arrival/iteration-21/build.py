#!/usr/bin/env python3
"""Build one instance of the work "Arrival", iteration 21, from public USGS data.
...
ORDER_BANDS = ((1.00, 1.25), (1.25, 1.50), (1.50, 2.00),
               (2.00, 3.00), (3.00, 5.00), (5.00, None))


def band_of(ratio):
    """Which separation band a pair of distances falls in, by their ratio.

    The same bands `order_by_distance` counts in, asked of one pair. Added by
    iteration 19, which needs to say how many pairs change band when the record
    moves the origin the ratio is taken from.
    """
    for j, (a, b) in enumerate(ORDER_BANDS):
        if ratio >= a and (b is None or ratio < b):
            return j
    return len(ORDER_BANDS) - 1


def order_by_distance(rows):
    """Does the nearer block carry the greater intensity, and at what separation?

    `rows` is (distance, intensity) for the blocks standing at the last version.
    Returns the whole count and the same count split by the ratio of the farther
    block's distance to the nearer one's — holds / fails / tied, where tied
    means the two intensities are equal and is never folded into either side.

    Every pair is counted; nothing is sampled and no record is too large for
    this, because the count is taken from a running tally over the intensities
    already passed rather than by visiting pairs. Sorted by distance, the blocks
    standing at a given ratio from one block are a contiguous run of that order
    whose two ends only move forward — so each band is one window that slides.
    Added by iteration 18.
    """
    rows = sorted(rows, key=lambda r: r[0])
    n = len(rows)
    vals = sorted({c for _d, c in rows})
    rank = {v: i for i, v in enumerate(vals)}
    out = {}

    def tally(window):
        """holds, fails, tied over the pairs `window` admits."""
        tree = [0] * (len(vals) + 1)
        ...
        lo = hi = 0
        for j in range(n):
            keep_lo, keep_hi = window(rows[j][0])
            while hi < n and rows[hi][0] <= keep_hi and rows[hi][0] < rows[j][0]:
                add(rank[rows[hi][1]], 1)
                hi += 1
            while lo < hi and rows[lo][0] <= keep_lo:
                add(rank[rows[lo][1]], -1)
                lo += 1
            r = rank[rows[j][1]]
            inside = upto(len(vals) - 1)
            le = upto(r)
            lt = upto(r - 1) if r else 0
            holds += inside - le
            fails += lt
            tied += le - lt
        return [holds, fails, tied]

    out["all"] = tally(lambda d: (-1.0, d))
    out["bands"] = [
        [a, b, *tally(lambda d, a=a, b=b: (0.0 if b is None else d / b, d / a))]
        for a, b in ORDER_BANDS]
    out["blocks"] = n
    return out
...
def build(eventid=None):
    ...
    # **What iteration 21 adds, and it is the only thing it adds.**
    #
    # Since its fifteenth iteration this file has said how many blocks standing
    # now the record ever published at a different intensity, and how many such
    # changes there are between them. Two numbers a reader can divide, and the
    # quotient is a mean — which is the one shape this history does not have.
    # On 2026-09-19 the entire movement of one of this work's six records over a
    # day was one square's ninth revision, 38.8 days after the earthquake: the
    # record had returned to the same piece of ground nine times while leaving
    # 698 others alone. Asked how deep it goes on one block, the file could not
    # answer.
    #
    # So the depth is measured here. Over every block this record ever
    # published — a denominator of its own, and named as its own in the file,
    # because the sentence beside it counts only blocks standing now — how many
    # were republished at all, how many more than once, how often the deepest
    # was republished, and how long after its first publication the last of
    # those came. The deepest is chosen by the count of republications, ties
    # broken by the longer span and then by the lower index, so that two builds
    # of one record name the same block.
    #
    # A republication here is what the version diff above already calls one: a
    # version in which the record published a block it was already publishing,
    # at a different reporter count or a different intensity. Nothing else
    # changes.
    rev_depth, first_min, last_min = {}, {}, {}
    for ver in versions:
        for i, _n, _c in ver["a"]:
            first_min.setdefault(i, ver["min"])
        for i, _n, _c in ver["c"]:
            rev_depth[i] = rev_depth.get(i, 0) + 1
            last_min[i] = ver["min"]
    if rev_depth:
        deepest = max(
            rev_depth,
            key=lambda i: (rev_depth[i],
                           round(last_min[i] - first_min.get(i, last_min[i]), 2),
                           -i))
        felt_depth = {
            "blocks": len(first_min),
            "ever": len(rev_depth),
            "deep": sum(1 for k in rev_depth.values() if k > 1),
            "max": rev_depth[deepest],
            "i": deepest,
            "spanMin": round(last_min[deepest]
                             - first_min.get(deepest, last_min[deepest]), 2),
        }
    else:
        felt_depth = {"blocks": len(first_min), "ever": 0, "deep": 0,
                      "max": 0, "i": None, "spanMin": 0.0}
    ...
    # How finely this record's own quantity orders the axis the figure draws it
    # on. Added by iteration 18.
    #
    # The blocks standing at the last version, each at the coordinate this file
    # gives it — the midpoint of its published extent against the epicentre its
    # own record last published it under, which is the rule iteration 15 fixed
    # and the rule the figure draws by. For every pair whose two distances
    # differ: the pair *holds* where the nearer block carries the strictly
    # greater intensity, *fails* where it carries the strictly smaller, and is
    # *tied* where the two are equal. Ties are counted and never redistributed.
    # Split by how far apart the two blocks are, as the ratio of the farther
    # distance to the nearer.
    #
    # Why it is in the file at all: this practice measured the same thing on two
    # events on 2026-08-30 and published it as a finding about the felt record,
    # and on 2026-09-13 ran it over a population, where the direction held and
    # the numbers did not. A reader of the figure has no way to know at what
    # separation its horizontal axis is readable unless the file says so, and
    # the file can only say so honestly about the record it is drawn from.
    # See `ledger/2026-09-13-session-29-what-a-claim-of-three-events-was.md`.
    ord_st, ord_e, ord_c = {}, {}, {}
    for ver in versions:
        for i, n, c in ver["a"]:
            ord_st[i] = n
    ...
    for i in ord_st:
        e = ord_e[i]
        lo_i, hi_i = blocks[i]["lo"][e], blocks[i]["hi"][e]
        if lo_i is None or hi_i is None:
            continue
        ord_rows.append(((lo_i + hi_i) / 2.0, ord_c[i]))
    felt_order = order_by_distance(ord_rows)

    # Iteration 18 printed at what separation this half can be read, and session
    # 29 wrote down beside it what a reader of that paragraph is not told: the
    # bands are ratios of distances measured from *an* epicentre, and this
    # record publishes more than one. The overall count hardly moves when the
    # origin does — it is an ordering of the same blocks — but the split by
    # separation moves a lot, because every pair's band is a ratio taken from
    # wherever the origin now is.
    #
    # On 2026-09-16 the record showed it rather than the practice arguing it: the
    # Peruvian record published a fourth epicentre, 3.811 km from the third, and
    # the file's overall count stood at exactly 926 of 1 308 before and after
    # while 32 of 1 431 pairs changed band. So the file says it about itself,
    # from its own record: the same pairs, the same intensities, measured from
    # the first epicentre this record ever published instead of the one in
    # force. Nothing is smoothed and nothing is corrected — the figures above
    # stay the ones the record in force gives.
    #
    # **What iteration 20 adds, and it is the only thing it adds.**
    #
    # Iteration 19 measured the shift against the *first* published epicentre,
    # and session 30 named the omission that leaves standing, in the same shape
    # as the one iteration 19 had just repaired, one level down: the first
    # epicentre is not the worst one. A record that published four origins
    # offers three readings of its own horizontal axis that are not the one in
    # force, and the file reported one of them and did not say it was reporting
    # the smallest, the largest, or an arbitrary one. So the same measure is now
    # taken against **every** published epicentre that is not the one in force,
    # and the one that moves the most pairs is reported beside the first. The
    # first is kept — it is the record's own starting point and not an
    # arbitrary pick — and the worst is what bounds the reading.
    rev = {i: bk for bk, i in order.items()}
    felt_shift = None
    if len(epis) > 1 and ord_rows:
        now_d = [d for d, _c in ord_rows]

        def _from_epi(ep):
            """The same blocks at the same intensities, measured from `ep`.

            Returns None where a block standing in the ordering has no outline
            to measure, which is the condition iteration 19 tested by length.
            """
            rows = []
            for i in ord_st:
                bk = rev[i]
                if bk not in rings:
                    continue
                lo0, hi0 = outline_edges(rings[bk], ep["lat"], ep["lon"])
                rows.append(((lo0 + hi0) / 2.0, ord_c[i]))
            return rows if len(rows) == len(ord_rows) else None

        def _band_change(rows):
            """Pairs falling in a different band, this origin against the one in
            force, and the pairs on which the question can be asked at all."""
            was_d = [d for d, _c in rows]
            # Named `band` and not `changed`: through iterations 19 and 20 this
            # local was called `changed`, and it stood in the same scope as the
            # count of block republications far above it, which the payload
            # publishes as `counts.changed`. The second assignment reached the
            # first. Iteration 21 found it by computing the same quantity again
            # for its own measurement and getting 14 where the payload said 239.
            band = pairs = 0
            for j in range(len(now_d)):
                for k in range(j):
                    a1, b1 = now_d[j], now_d[k]
                    a0, b0 = was_d[j], was_d[k]
                    if a1 == b1 or a0 == b0:
                        continue
                    pairs += 1
                    r1 = a1 / b1 if a1 > b1 else b1 / a1
                    r0 = a0 / b0 if a0 > b0 else b0 / a0
                    if band_of(r1) != band_of(r0):
                        band += 1
            return band, pairs

        first = _from_epi(epis[0])
        if first is not None:
            band, pairs = _band_change(first)
            felt_shift = {
                "epis": len(epis),
                "km": round(gc_km(epis[0]["lat"], epis[0]["lon"],
                                  epis[-1]["lat"], epis[-1]["lon"]), 3),
                "changed": band, "pairs": pairs,
                **{k: v for k, v in order_by_distance(first).items()
                   if k in ("all", "bands")},
            }
            # Every other published origin, the one in force excepted, and the
            # one that moves the most pairs. `i` is the origin's index in the
            # published order, so a reader can see whether the worst reading is
            # the record's first or one it passed through.
            worst = {"i": 0, "km": felt_shift["km"],
                     "changed": band, "pairs": pairs}
            for idx in range(1, len(epis) - 1):
                rows = _from_epi(epis[idx])
                if rows is None:
                    continue
                c, p = _band_change(rows)
                if c > worst["changed"]:
                    worst = {
                        "i": idx,
                        "km": round(gc_km(epis[idx]["lat"], epis[idx]["lon"],
                                          epis[-1]["lat"], epis[-1]["lon"]), 3),
                        "changed": c, "pairs": p,
                    }
            felt_shift["worst"] = worst
    ...
    payload = {
        ...
        "feltOrder": felt_order,
        "feltShift": felt_shift,
        "feltDepth": felt_depth,
    ...
    if felt_depth["max"]:
        print(f"         the felt record returned to one block"
              f" {felt_depth['max']} time(s) — block {felt_depth['i']},"
              f" last of them +{felt_depth['spanMin']:.2f} min"
              f" = +{felt_depth['spanMin']/1440:.2f} d after it was first"
              f" published; {felt_depth['deep']} of"
              f" {felt_depth['blocks']} block(s) ever published were"
              f" republished more than once")
    ...
    oh, of_, ot = felt_order["all"]
    print(f"         the nearer block is the stronger on {oh} of {oh + of_}"
          f" pair(s) this record orders ({100.0 * oh / (oh + of_):.1f} %)"
          f" over {felt_order['blocks']} standing block(s), {ot} tied")
    for a, b, bh, bf, bt in felt_order["bands"]:
        lab = f"{a:.2f}-{b:.2f}x" if b is not None else f"{a:.2f}x+"
        pc = f"{100.0 * bh / (bh + bf):.1f} %" if bh + bf else "    --"
        print(f"           {lab:>12s} {pc:>7s} on {bh + bf} pair(s), {bt} tied")
    if felt_shift:
        print(f"         the same pairs from the first of {felt_shift['epis']}"
              f" epicentre(s), {felt_shift['km']} km away:"
              f" {felt_shift['all'][0]}/{felt_shift['all'][0] + felt_shift['all'][1]}"
              f" overall against {felt_order['all'][0]}/"
              f"{felt_order['all'][0] + felt_order['all'][1]};"
              f" {felt_shift['changed']} of {felt_shift['pairs']} pairs"
              f" change band")
        w = felt_shift.get("worst")
        if w:
            print(f"         worst of the {felt_shift['epis'] - 1} origin(s) it"
                  f" left: #{w['i'] + 1} at {w['km']} km,"
                  f" {w['changed']} of {w['pairs']} pairs change band")
